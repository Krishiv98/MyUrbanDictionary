...
@Component
export default class SearchBar extends Mixins(GlobalMixin) {
  ...
  // eslint-disable
  // data variables are constantly monitored for changes
  // and when their values change Vue 'refreshes' the display
   ...
   * Trigger an event on the current instance. Any
   * additional arguments will be passed into the listener's callback function.
   ...
   * Watches MUST correspond to a data variable or prop we are watching -
   * in this example below, the 'query' data variable
   ...
   */
  @Watch('query')
  onQueryChanged(newVal: string, oldVal: string):void {
    // if the student list is hidden or if the user is removing chars then don't call the api
    if (!this.showList || newVal.length < oldVal.length) { return; }
    ...
    this.callAPI(this.URBAN_TERM_API, 'get', { _sort: 'familyName', _order: 'asc', q: this.query })
      ...
      .finally(() => {
        this.setBusy(false); // tell parent component
        // that this component is no longer waiting for the api
      });
  }
}
</script>
